...
func TestMigrateToUniqueEventIndex_NonUnique(t *testing.T) {
	var statements []string
	mockTx := &utils.MockTx{
		ExecuteFn: func(ctx context.Context, stmt string, args ...any) (*kwilsql.ResultSet, error) {
			statements = append(statements, stmt)
			// Simulate pg_index query returning non-unique (true)
			if strings.Contains(stmt, "pg_index") {
	...
	err := migrateToUniqueEventIndex(context.Background(), mockTx)
	require.NoError(t, err)

	// Should have run: pg_index check, DELETE dedup, DROP INDEX
	require.True(t, containsSQL(statements, "pg_index"), "should check pg_index")
	require.True(t, containsSQL(statements, "DELETE FROM"), "should deduplicate rows")
	require.True(t, containsSQL(statements, "DROP INDEX"), "should drop non-unique index")
}

func TestMigrateToUniqueEventIndex_AlreadyUnique(t *testing.T) {
	var statements []string
	mockTx := &utils.MockTx{
		ExecuteFn: func(ctx context.Context, stmt string, args ...any) (*kwilsql.ResultSet, error) {
			statements = append(statements, stmt)
			// Simulate pg_index query returning already unique (false)
			if strings.Contains(stmt, "pg_index") {
				return &kwilsql.ResultSet{
					Columns: []string{"?column?"},
					Rows:    [][]any{{false}},
	...
	// Should only have the pg_index check, no dedup or drop
	require.Len(t, statements, 1, "should only check pg_index, no further action")
}

func TestMigrateToUniqueEventIndex_FreshInstall(t *testing.T) {
	var statements []string
	mockTx := &utils.MockTx{
		ExecuteFn: func(ctx context.Context, stmt string, args ...any) (*kwilsql.ResultSet, error) {
			statements = append(statements, stmt)
			// Simulate pg_index query returning no rows (index doesn't exist)
			if strings.Contains(stmt, "pg_index") {
