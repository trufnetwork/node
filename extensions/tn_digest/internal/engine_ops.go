...
// TrimTransactionEventsTxResult represents the parsed result from a
// trim_transaction_events transaction.
type TrimTransactionEventsTxResult struct {
	Deleted   int
	Remaining int
	HasMore   bool
}

// BroadcastTrimTransactionEventsWithRetry wraps broadcastTrimTransactionEventsOnce
// with retry logic. On any error it re-fetches a fresh nonce before retrying,
// mirroring BroadcastTrimOrderEventsWithRetry.
func (e *EngineOperations) BroadcastTrimTransactionEventsWithRetry(
	ctx context.Context,
	chainID string,
	signer auth.Signer,
	broadcaster func(context.Context, *ktypes.Transaction, uint8) (ktypes.Hash, *ktypes.TxResult, error),
	...
	deleteCap int,
	maxRetries int,
) (*TrimTransactionEventsTxResult, error) {
	var lastErr error
	backoff := 5 * time.Second
	maxBackoff := 30 * time.Second
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		if attempt > 0 {
			e.logger.Warn("retrying trim_transaction_events with fresh nonce",
				"attempt", attempt, "last_error", lastErr)
		...
		result, err := e.broadcastTrimTransactionEventsOnce(ctx, chainID, signer, broadcaster, preserveBlocks, deleteCap)
		if err == nil {
			return result, nil
		}
		lastErr = err
	...
	return nil, fmt.Errorf("trim_transaction_events max retries (%d) exceeded: %w", maxRetries, lastErr)
}

// broadcastTrimTransactionEventsOnce fetches a fresh nonce and broadcasts once.
func (e *EngineOperations) broadcastTrimTransactionEventsOnce(
	ctx context.Context,
	chainID string,
	signer auth.Signer,
	broadcaster func(context.Context, *ktypes.Transaction, uint8) (ktypes.Hash, *ktypes.TxResult, error),
	...
	payload := &ktypes.ActionExecution{
		Namespace: "main",
		Action:    "trim_transaction_events",
		Arguments: [][]*ktypes.EncodedValue{{
			preserveBlocksArg, deleteCapArg,
		}},
	}
	...
	result, err := parseTxEventsTrimResultFromTxLog(txResult.Log)
	if err != nil {
		e.logger.Warn("failed to parse trim_transaction_events result", "error", err, "log", txResult.Log)
		return nil, fmt.Errorf("parse trim result: %w", err)
	}

	e.logger.Info("trim_transaction_events completed",
		"deleted", result.Deleted,
		"remaining", result.Remaining,
		"has_more", result.HasMore,
		"tx_hash", hash.String())
...
// parseTxEventsTrimResultFromTxLog extracts deleted/remaining/has_more from NOTICE
// output, mirroring parseTrimResultFromTxLog but keyed on the
// trim_transaction_events marker.
func parseTxEventsTrimResultFromTxLog(logOutput string) (*TrimTransactionEventsTxResult, error) {
	result := &TrimTransactionEventsTxResult{}
	for _, line := range strings.Split(logOutput, "\n") {
		if !strings.Contains(line, "trim_transaction_events:") {
			continue
		}
		// Extract payload after marker (handles prefixed lines like "NOTICE: trim_transaction_events: ...")
		parts := strings.SplitN(line, "trim_transaction_events:", 2)
		if len(parts) != 2 {
			continue
		}
		payload := strings.TrimSpace(parts[1])
	...
	return nil, fmt.Errorf("no trim_transaction_events log entry found in: %q", logOutput)
}
