...
| **Single-node + admin RPC on `127.0.0.1:8485`** (for `tn_local` SDK / on-host admin tooling) | `task single:start:admin` → `task single:stop` |
...
##### Enabling admin RPC for `tn_local` and on-host admin tooling

`task single:start` keeps the kwild admin JSON-RPC bound to a unix socket
inside the container — kwild's safe default. To run the `tn_local` SDK
examples (`sdk-go`/`sdk-py`/`sdk-js` `examples/local_actions_example/`),
or any other on-host admin work, use the opt-in variant:
...
transport from the kwild docs:
[Administration of a Running Node](https://kwil-docs.vercel.app/docs/admin/node)
covers the TLS / mTLS / `admin.pass` setup. Plain HTTP on a public
interface (`--admin.notls` + `0.0.0.0`) is never appropriate.

##### `tn_local`-specific application auth (`require_signature`)

This is the only auth layer that's TN-specific — kwild's transport auth
(see the link above) protects the wire; this protects the operator
identity. With `require_signature = "true"` set, every `local.*` call
must carry a signature recovered to the node's own secp256k1 address.

```toml
# /root/.kwild/config.toml — only TN_LOCAL keys, kwild's admin block is separate
[extensions.tn_local]
require_signature = "true"
replay_window_seconds = "60"   # optional, default 60
```

Behavior when enabled:

- Every request needs a `_auth` envelope (`sig`, `ts`, `ver`) signed by
  the node operator's secp256k1 key. SDKs with the operator key
  configured (e.g. `tnclient.WithLocalSigner(priv)` in sdk-go) attach it
  per call — users never call `.sign()` themselves.
- Server recovers the signing address from the signature; rejects unless
  it equals the node address. Replay-protected within
  `replay_window_seconds`. Unsigned calls return
  `tn_local: unauthenticated` (`-32602`).

For the dev node, the override flag is wired to an env var:

```shell
TN_LOCAL_REQUIRE_SIGNATURE=true task single:start:admin
```

The autogenerated operator key lives at `/root/.kwild/nodekey.json` in
the container — extract it for the SDK examples with
