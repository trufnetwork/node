...
func InitializeFoodStream(ctx *execution.DeploymentContext, metadata map[string]string) (execution.ExtensionNamespace, error) {
...
var _ = execution.ExtensionInitializer(InitializeFoodStream)

type FoodStreamExt struct {
	table         string
	dateColumn    string
	cpiColumn     string
	yahooColumn   string
...
func (b *FoodStreamExt) Call(scope *execution.ProcedureContext, method string, args []any) ([]any, error) {
...
const (
	sqlGetBaseValue = `SELECT 	
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s ASC LIMIT 1) AS cpi,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s ASC LIMIT 1) AS yahoo,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s ASC LIMIT 1) AS nielsen,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s ASC LIMIT 1) AS numbeo;`
	sqlGetLatestValue = `SELECT
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s DESC LIMIT 1) AS cpi,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s DESC LIMIT 1) AS yahoo,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s DESC LIMIT 1) AS nielsen,
		(SELECT %s FROM %s WHERE %s !=0 ORDER BY %s DESC LIMIT 1) AS numbeo;`
...
func (b *FoodStreamExt) sqlGetBaseValue() string {
	return fmt.Sprintf(sqlGetBaseValue, b.cpiColumn, b.table, b.cpiColumn, b.dateColumn, b.yahooColumn, b.table, b.yahooColumn, b.dateColumn, b.nielsenColumn, b.table, b.nielsenColumn, b.dateColumn, b.numbeoColumn, b.table, b.numbeoColumn, b.dateColumn)
}

func (b *FoodStreamExt) sqlGetLatestValue() string {
	return fmt.Sprintf(sqlGetLatestValue, b.cpiColumn, b.table, b.dateColumn, b.cpiColumn, b.yahooColumn, b.table, b.dateColumn, b.yahooColumn, b.nielsenColumn, b.table, b.dateColumn, b.nielsenColumn, b.numbeoColumn, b.table, b.dateColumn, b.numbeoColumn)
}

func (b *FoodStreamExt) sqlGetSpecificValue() string {
...
func (b *FoodStreamExt) index(ctx context.Context, dataset Querier, date string) (int64, error) {
...
func (b *FoodStreamExt) value(ctx context.Context, dataset Querier, date string) (int64, error) {
