...
func Test_Index(t *testing.T) {
	...
	assert.NoError(t, err)
	returned2, err := b.index(ctx, mockQ, zeroDate, nil) // this should return the latest value
	assert.NoError(t, err)
...
func Test_Value(t *testing.T) {
	...
	assert.NoError(t, err)
	returned2, err := b.value(ctx, mockQ, zeroDate, nil) // this should return the latest value
	assert.NoError(t, err)
	assert.Equal(t, int64(200000), returned) // 200.000 * 1000
