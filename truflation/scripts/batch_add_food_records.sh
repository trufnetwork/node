...
for file in ./temp_csv/*.csv; do
  if echo "$file" | grep -q "Cereal" && echo "$file" | grep -q "cpi"; then
    ...
    echo "Processing file: $file"
    kwil-cli database batch --path "$file" --action add_record --name meat_cpi --values created_at:$(date +%s)
  elif echo "$file" | grep -q "Meats" && echo "$file" | grep -q "nielsen"; then
    echo "Processing file: $file"
    kwil-cli database batch --path "$file" --action add_record --name meat_nielsen --values created_at:$(date +%s)
  elif echo "$file" | grep -q "Meats" && echo "$file" | grep -q "numbeo"; then
    echo "Processing file: $file"
    kwil-cli database batch --path "$file" --action add_record --name meat_numbeo --values created_at:$(date +%s)
  elif echo "$file" | grep -q "Meats" && echo "$file" | grep -q "yahoo"; then
    echo "Processing file: $file"
    kwil-cli database batch --path "$file" --action add_record --name meat_yahoo --values created_at:$(date +%s)
