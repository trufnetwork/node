...
func TestERC20BridgeTransferBalances(t *testing.T) {
	seedAndRun(t, "erc20_bridge_transfer_balances", "simple_mock.sql", func(ctx context.Context, platform *kwilTesting.Platform) error {
		...
		balanceA, err := testerc20.GetUserBalance(ctx, platform, TestUserA)
		require.NoError(t, err)
		require.Equal(t, TestAmount2, balanceA, "userA should have full deposit amount")

		// Verify userB has zero balance initially
		balanceB, err := testerc20.GetUserBalance(ctx, platform, TestUserB)
		require.NoError(t, err)
		...
		_, err = platform.Engine.Call(engineCtx, platform.DB, "sepolia_bridge", "transfer", []any{TestUserB, halfDec}, func(row *common.Row) error {
			return nil
		})
		require.NoError(t, err)
