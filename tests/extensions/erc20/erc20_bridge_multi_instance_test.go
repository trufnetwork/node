...
func TestERC20BridgeMultiInstanceIsolation(t *testing.T) {
	seedAndRun(t, "erc20_bridge_multi_instance_isolation", "", func(ctx context.Context, platform *kwilTesting.Platform) error {
		...
		`, TestChain, escrowA), nil, nil)
		require.NoError(t, err)

		// Create aliasB for escrowB
		err = app.Engine.ExecuteWithoutEngineCtx(ctx, app.DB, fmt.Sprintf(`
			USE erc20 {
				chain: '%s',
				escrow: '%s'
			} AS aliasB
		`, TestChain, escrowB), nil, nil)
		require.NoError(t, err)

		// Step 1: Inject deposit only for escrowA, userX
		// This simulates a deposit to escrowA only
		err = testerc20.InjectERC20Transfer(ctx, app, TestChain, escrowA, TestERC20, TestUserA, escrowA, TestAmount1, 10, nil)
		require.NoError(t, err)
		...
		testerc20.DeactivateCurrentInstanceTx(t, platform, escrowA)
		testerc20.DeactivateCurrentInstanceTx(t, platform, escrowB)
