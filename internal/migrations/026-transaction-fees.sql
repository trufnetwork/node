/**
 * MIGRATION 026: TRANSACTION FEE COLLECTION - STREAM CREATION
 *
 * Adds fee collection to stream creation operations with role-based exemption.
 * - Flat 2 TRUF fee for stream creation (per create_streams call, not per stream)
 * - Wallets with system:network_writer role are exempt from fees
 * - Fee transferred to @leader_sender (block proposer)
 *
 * Changes from original (001-common-actions.sql):
 * - Role check changed from REQUIREMENT to EXEMPTION
 * - Fee collection added before business logic
 * - Non-exempt users can now create streams (previously blocked)
 */

-- =====================================================================
-- STREAM CREATION WITH FEE COLLECTION
-- =====================================================================

/**
 * create_streams: Creates multiple streams at once.
 * Fee: 2 TRUF flat fee (one fee per call, regardless of number of streams)
...
/**
 * create_stream: Creates a new stream (delegates to create_streams)
 * Fee: 2 TRUF flat fee (collected by create_streams)
 * Exemption: system:network_writer role bypasses fee collection
 */
