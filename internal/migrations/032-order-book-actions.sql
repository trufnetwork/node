...
CREATE OR REPLACE ACTION place_split_limit_order(
    ...
    -- ==========================================================================
    -- SECTION 8: LP ELIGIBILITY TRACKING (ISSUE 9A)
    -- ==========================================================================

    -- Track liquidity providers for fee distribution rewards.
    -- LPs qualify by placing split orders with tight spreads and sufficient size.
    --
    -- Eligibility Criteria:
    -- 1. Spread: BOTH buy and sell prices within max_spread of midpoint (50 cents)
    -- 2. Size: Order amount >= min_order_size
    --
    -- Qualification is snapshot-based: Determined at order placement time.
    -- Later changes to market parameters do not retroactively affect LP status.

    -- Step 1: Get market parameters
    $market_max_spread INT;
    $market_min_order_size INT8;

    for $row in SELECT max_spread, min_order_size FROM ob_queries WHERE id = $query_id {
        $market_max_spread := $row.max_spread;
        $market_min_order_size := $row.min_order_size;
    }

    -- Validate market parameters were retrieved (defensive check)
    if $market_max_spread IS NULL OR $market_min_order_size IS NULL {
        ERROR('Market parameters (max_spread, min_order_size) not properly configured');
    }

    -- Step 2: Calculate price distances from midpoint (50 cents)
    -- Price semantics: Prices are in cents (0-100), midpoint is always 50
    $buy_distance INT := abs(50 - $true_price);    -- Distance of YES buy price
    $sell_distance INT := abs(50 - $false_price);  -- Distance of NO sell price

    -- Step 3: Check qualification criteria
    -- Qualifies if BOTH prices are tight AND amount is sufficient
    $qualifies BOOL :=
        ($buy_distance <= $market_max_spread) AND
        ($sell_distance <= $market_max_spread) AND
        ($amount >= $market_min_order_size);

    -- Step 4: Track LP if qualified
    -- Uses UPSERT pattern: Insert new record or accumulate volume for existing LP
    if $qualifies {
        INSERT INTO ob_liquidity_providers (
            query_id,
            participant_id,
            split_order_amount,
            last_order_true_price,
            last_order_false_price,
            first_qualified_at,
            last_qualified_at,
            total_qualified_orders
        ) VALUES (
            $query_id,
            $participant_id,
            $amount,
        ...
        ON CONFLICT (query_id, participant_id) DO UPDATE
        SET split_order_amount = ob_liquidity_providers.split_order_amount + EXCLUDED.split_order_amount,
            last_order_true_price = EXCLUDED.last_order_true_price,
            last_order_false_price = EXCLUDED.last_order_false_price,
            last_qualified_at = EXCLUDED.last_qualified_at,
            total_qualified_orders = ob_liquidity_providers.total_qualified_orders + 1;
    }

    -- LP tracking complete (Issue 9B will distribute fees to tracked LPs)
};
